...
namespace resonances0even_az
{
  ...
  /////////////////////////////// Z gam final state from OZg operator

  // Computes the exotic spin 0 even  squared matrix element and the SM interference
  double
  sqme( double s, double t, int exclude_loops_SM, int exclude_loops_EX, double m, double c, double czg, double w_c, double aa )
  {
    double mass = m;
    double f0 = c;
    double f0Zg = czg;
    ...
    // read in input data
    /*ifstream data( "./data_resonance_Zg" );
    data >> mass >> f0 >> f0Zg >> w_const >> a2;
    data.close();
    */
    ...
    MZgpppp_spin0even(s, t, mass, f0, f0Zg, w_const,a2, re_ex, im_ex);
    ...
    // Mppmm:
    MZgppmm_spin0even(s, t, mass, f0, f0Zg, w_const,a2, re_ex, im_ex);
    me_SM(Mppmm_fermion,s,t, re_SM, im_SM, exclude_loops_SM);
    value += re_ex*(re_ex+2*re_SM) + im_ex*(im_ex+2*im_SM) ;

    // Mpmmp:
    MZgpmmp_spin0even(s, t, mass, f0, f0Zg, w_const,a2, re_ex, im_ex);
    me_SM(Mpmmp_fermion,s,t, re_SM, im_SM, exclude_loops_SM);
    value += re_ex*(re_ex+2*re_SM) + im_ex*(im_ex+2*im_SM) ;

    // Mpmpm:
    MZgpmpm_spin0even(s, t, mass, f0, f0Zg, w_const,a2, re_ex, im_ex);
    me_SM(Mpmpm_fermion,s,t, re_SM, im_SM, exclude_loops_SM);
    value += re_ex*(re_ex+2*re_SM) + im_ex*(im_ex+2*im_SM) ;

    // Mpppm
    MZgpppm_spin0even(s, t, mass, f0, f0Zg, w_const,a2, re_ex, im_ex);
