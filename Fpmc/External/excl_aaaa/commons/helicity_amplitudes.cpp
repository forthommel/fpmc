...
enum region_t { low = 1, high = 2, forward = 3, backward = 4 };

int
limits( double sred, double tred, double ured )
{
...
Mpppp_fermion( double sred, double tred, double& re, double& im, int exclude_loops )
{
  ...
  if ( region==low ){ // EFT limit
    ...
    re = 1.+(tred-ured)/sred * log(tred/ured)+(tred*tred+ured*ured) / (2.*sred*sred)*(pow( log(tred/ured) , 2 )+M_PI*M_PI);
    im = 0;
  }
...
Mpppp_vector( double sred, double tred, double& re, double& im, int exclude_loops )
{
  ...
  if( region == low ){ // EFT limit
    ...
    im = ( 2. * M_PI * sred*sred *( log(-4.*ured)  / (sred * ured) +
...
Mpmpm_vector( double sred, double tred, double& re, double& im, int exclude_loops )
{
  ...
  if( region == low ) { // EFT limit 
    ...
    im = - ( 1.5 * (sred-tred)/ured * (- M_PI)+ 
      ...
      2. * (-M_PI) * ured*ured*  ( log(-4.*ured) / (ured*sred) +
...
{
  return 1./m*( a2*s*s / ( 4*M_PI*f0*f0 ) ) +w_const;
}
